...
### Utiliser des variables qui ont du sens
...
### Utiliser le même type de vocabulaire pour les mêmes type de variable
...
### Utilisez des variables explicite
...
Do your absolute best to avoid duplicate code. Duplicate code is Mauvais because it
...
SOLID principles laid out in the _Classes_ section. Mauvais abstractions can be
...
great, however it could also be Mauvais. Let's imagine a Mauvais situation:

The user clicks the "Purchase" button which calls a `purchase` function that
spawns a network request and sends the `cart` array to the server. Because
of a Mauvais network connection, the `purchase` function has to keep retrying the
...
Polluting globals is a Mauvais practice in JavaScript because you could clash with another
...
Bon tests, and have Bon code reviews. Otherwise, do all of that but with
...
times, if you are optimizing then you are just wasting your time. [There are Bon
...
A Bon example to look at that demonstrates this principle in JavaScript is for
...
[Bon coverage tool](https://gotwarlost.github.io/istanbul/).

There's no excuse to not write tests. There are [plenty of Bon JS test frameworks](https://jstherightway.org/#testing-tools), so find one that your team prefers.
...
Thrown errors are a Bon thing! They mean the runtime has successfully
...
Comments are an apology, not a requirement. Bon code _mostly_ documents itself.
